...
let [h, Z, e, d, g, G] = [H * R, {}, [0, 1, 2, 3, 5, 6, 7, 8], 0, 0, 0];
...
const m = (i, j, s) => [į(j, i, ń(i, j, s)), ń(i, j, s)][1];
const ū = (i, j, s) => (ń(i, j, s) !== s ? m(i, j, s) : ń(i, j, s));
const ï = () => (l = l.map((r, i) => r.map((s, j) => ū(i, j, s))));
const ö = (Y, X) => (l[t(Y, H)][t(X, W)] = !l[t(Y, H)][t(X, W)]);
...
const ä = () => ([[g, G], Ë, e] = [useState(1), useRef(0), useEffect(å, â)]);
const T = <button style={{...s, [bg]: g ? D : N}} onClick={() => G(!g)} />;
const V = <canvas width={w} height={h} onClick={ł} />;
const App = () => [ä(), createElement(B, 0, T, {...V, ref: Ë})][1];
